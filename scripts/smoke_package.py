...
def main() -> int:
    ...
    parser.add_argument("--profile", choices=("core", "cli", "mcp", "ui", "benchmark", "sdist"), required=True)
    args = parser.parse_args()
    files = validate(args.dist_dir, args.version)
    verify_checksums(files, args.checksums)
    artifact = next(p for p in files if p.name.endswith(".tar.gz" if args.profile == "sdist" else ".whl"))
    smoke(artifact, args.version, "core" if args.profile == "sdist" else args.profile)
