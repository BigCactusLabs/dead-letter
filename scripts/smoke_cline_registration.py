...
# Test the official native package on the named CI platform. The cline npm
# wrapper did not expose an executable in our clean npx run; do not silently
# substitute this result for wrapper-install acceptance. Source: cline/cline
# apps/cli/script/build.ts at d48afb3542b07cbb6d17b40d7c536aadc9cbf041.
CLINE_PACKAGE = "@cline/cli-linux-x64"
...
def check() -> dict:
    ...
    with tempfile.TemporaryDirectory(prefix="dead-letter-cline-") as temporary:
        ...
        cli = [npx, "--yes", f"--package={CLINE_PACKAGE}@{CLINE_VERSION}", "cline"]
    ...
    return {"client": "cline", "package": CLINE_PACKAGE, "version": CLINE_VERSION, "registration": "passed", "saved_transport_smoke": "passed", "uninstall_preserves_settings": "passed", "scope": "disposable CLI profile; no GUI, model request, or marketplace submission"}
